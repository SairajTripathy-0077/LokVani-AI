...
import { queryDatabaseRAG } from './aiCoreEngine.js';
  ...
  switch (intent) {
    ...
    case INTENTS.SOIL_ADVISORY: {
      ...
      if (soilPred.status === 'INVALID_OR_OUT_OF_RANGE_INPUT') {
        ...
        // High stakes fertilizer / chemical review flag
        requiresTrustReview = true;
    ...
    case INTENTS.CROP_PREDICTION: {
      ...
      // 2. Crop Model (uses soil details if present)
      const cropInput = {
        ...combinedParams,
    ...
    default: {
      // General Agriculture / RAG Fallback
      ...
      if (ragMatch) {
        ...
        structuredContextForLLM = `[GENERAL AGRICULTURAL QUERY] User asked: "${queryText}". Provide grounded agricultural advice.`;
        fallbackAnswerEn = `Regarding "${queryText}": For best crop yield, ensure 50kg DAP/acre during sowing, maintain adequate field drainage, and verify current Mandi commodity rates.`;
        fallbackAnswerHi = `"${queryText}" के संबंध में: फसल की अच्छी पैदावार के लिए बुवाई के समय प्रति एकड़ 50 किग्रा डीएपी डालें, खेत में जल निकासी रखें और नजदीकी मंडी भाव जांचें।`;
      }
    }
  }
...
ENGLISH: <english_response>`;
    ...
    const llmRes = await Promise.race([getRotatedGeminiResponse(prompt), timeoutPromise]);
    ...
    console.warn('[aiOrchestrator] Gemini LLM explanation fallback used:', err.message);
  }
