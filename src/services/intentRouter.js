...
export function classifyIntent(queryText = '', defaultLocation = 'Azamgarh, UP') {
  ...
  if (!district && defaultLocation) {
    district = defaultLocation.split(',')[0].trim();
  }

  // Extract numbers if present (e.g. "pH 6.5", "50kg Urea")
  const phMatch = text.match(/ph\s*([0-9]+(?:\.[0-9]+)?)/i);
